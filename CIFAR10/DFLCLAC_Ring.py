...
def  cifar10_dfl_clac_ring(
    ...
    topology: str = "Ring",
    dirName: str = "CIFAR10_DFL_CLAC_Ring",
):
    """
    Simulate DFL-CLAC on CIFAR-10 under a *Ring* (decentralized) topology.
    ...
    base_model = CIFAR10CNNModel().to(device)
    base_sd = base_model.state_dict()

    clients: List[nn.Module] = [CIFAR10CNNModel().to(device) for _ in range(num_clients)]
    for c in clients:
    ...
    for r in range(num_rounds):
        ...
        pseudo_global = CIFAR10CNNModel().to(device)
...
if __name__ == "__main__":
    cifar10_dfl_clac_ring(
        ...
        topology="Star",
        dirName="CIFAR10_DFL_CLAC_real_dacey",
    )
