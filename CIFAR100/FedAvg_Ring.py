...
from models import CIFAR100ModelCNN
...
def cifar100_fedavg_ring(alpha=0.5, num_classes=100, num_clients=20, num_rounds=20, num_epochs=20,
                        batch_size=32, cluster_num=5, num_components=50, selected_ratio=0.8, Allow_delay=60,
                        aggregation="FedAvg", topology="Star", dirName="CIFAR100_FedAvg"):
    ...
    logging.info(f"Starting Federated Learning with Ring Topology. Log file: {log_file}")
    ...
    global_model = CIFAR100ModelCNN().to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(global_model.parameters(), lr=0.001)

    clients = [CIFAR100ModelCNN().to(device) for _ in range(num_clients)]
    ...
    for round_num in range(num_rounds):
        ...
        for client_idx in valid_client_indices:
            ...
            local_model = CIFAR100ModelCNN().to(device)
            ...
            net_delay = random.uniform(0.0, 10.0)
        ...
        aggregation_clients = [client_idx for client_idx in valid_client_indices if
                               client_idx not in clients_with_delays_over_1min]

        round_end_time = time.time()
        # Ring Topology Aggregation
        aggregation_start_time = time.time()
        for client_idx in aggregation_clients:  # Only aggregate for clients that are not delayed
            left_neighbor = (client_idx - 1) % num_clients
            right_neighbor = (client_idx + 1) % num_clients

            client_state_dict = clients[client_idx].state_dict()
            left_state_dict = clients[left_neighbor].state_dict()
            right_state_dict = clients[right_neighbor].state_dict()

            for key in client_state_dict:
                client_state_dict[key] = (client_state_dict[key] + left_state_dict[key] + right_state_dict[key]) / 3

            clients[client_idx].load_state_dict(client_state_dict)
            global_model.load_state_dict(client_state_dict)

        global_model_size = calculate_model_size(global_model)
        print("global_model_size:", global_model_size)
        ...
        straggler_rate = sum([1 for delay in client_delays if delay > 30]) / len(
            client_delays)  # 30 seconds threshold for straggler
        ...
        metrics_dict['Effective Contribution Weight'].append(
            np.sum(client_delays))  # Placeholder for effective contribution weight
    ...
    model_path = os.path.join(log_file, "global_model_Cifar100_fedavg_ring.pth")
    torch.save(global_model.state_dict(), model_path)


if __name__ == "__main__":
    # Delay allows to clients Allow_delay
    cifar100_fedavg_ring(alpha=0.5, num_classes=100, num_clients=20, num_rounds=10, num_epochs=20,
                        batch_size=128, cluster_num=12, num_components=100, selected_ratio=0.9, Allow_delay=180,
                        aggregation="FedAvg", topology="Ring", dirName="CIFAR100_FedAvg_Ring")
